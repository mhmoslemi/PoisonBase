#!/usr/bin/env bash
# Submit the same 14 R < RAND configurations from the attack_if checkout.
...
PERSIST_DATA_ROOT=/home/mmoslem3/scratch/data
...
while IFS="$(printf '\t')" read -r config_id model attack class_pair budget rand_asr r_asr table_source; do
    ...
    job_name="rcna${config_id}_${model_tag}_${attack_tag}_${pair_tag}_b${budget_tag}"
...
echo "Prepared $job_count attack_if jobs on $ACCOUNT; walltime $WALLTIME each."
