#!/usr/bin/env bash
#
# Resume shard 1 of 4 for the killed dpp (SEL_ALPHA=2.0) sweep in dpp_crash.txt.
#
...
#   ResNet20BN  b0.04   26 victim trials + 4 crafts   ~1.14 h
#   VGG13BN     b0.02   12 victim trials + 1 craft    ~0.37 h
#   VGG13BN     b0.005   7 victim trials + 1 craft    ~0.22 h
#   ---------------------------------------------------------------
#   estimated total: 1.73 h        (the other 3 shards: 1.70-1.75 h)
#
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "d1.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

run () {   # run <model> <target_select> <budget>
    echo "=== d1 | dpp alpha=2.0 | $1 / fc / frog-airplane | budget $3 ==="
