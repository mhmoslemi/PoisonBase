#!/usr/bin/env bash
#
# Resume shard 2 of 4 for the killed dpp (SEL_ALPHA=2.0) sweep in dpp_crash.txt.
#
...
#   ResNet20BN  b0.001  22 victim trials + 3 crafts   ~0.87 h
#   ResNet20BN  b0.002  21 victim trials + 3 crafts   ~0.82 h
#   ---------------------------------------------------------------
#   estimated total: 1.70 h        (the other 3 shards: 1.70-1.75 h)
#
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "d2.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

run () {   # run <model> <target_select> <budget>
    echo "=== d2 | dpp alpha=2.0 | $1 / fc / frog-airplane | budget $3 ==="
...
run ResNet20BN  2 0.001
run ResNet20BN  2 0.002
