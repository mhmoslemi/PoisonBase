#!/usr/bin/env bash
#
# Resume shard 4 of 4 for the killed dpp (SEL_ALPHA=2.0) sweep in dpp_crash.txt.
#
...
#   ResNet20BN  b0.005  19 victim trials + 3 crafts   ~0.72 h
#   VGG13BN     b0.04   15 victim trials + 2 crafts   ~0.49 h
#   VGG13BN     b0.01    8 victim trials + 1 craft    ~0.25 h
#   VGG13BN     b0.002   8 victim trials + 1 craft    ~0.25 h
#   ---------------------------------------------------------------
#   estimated total: 1.71 h        (the other 3 shards: 1.70-1.75 h)
#
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "d4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

run () {   # run <model> <target_select> <budget>
    echo "=== d4 | dpp alpha=2.0 | $1 / fc / frog-airplane | budget $3 ==="
