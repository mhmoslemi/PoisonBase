...
#     MODEL=VGG13BN CLASS_PAIRS="dog-bird frog-airplane" sh sel_dpp_grad_.sh )
#
# CLASS_PAIRS is a space-separated LIST -- the difficulty label, the target file
# and the craft flags are all resolved per pair inside the loop, because
# sweep_config.json is keyed by a single pair name.  (CLASS_PAIR, singular, is
# still honoured so the old one-pair-at-a-time invocation keeps working.)
...
echo "=== dpp selection | $MODEL / $ATTACK | pairs: $CLASS_PAIRS | SEL_ALPHA=$SEL_ALPHA ==="
...
for pair in $CLASS_PAIRS; do
    ...
    # Resolved per pair: the difficulty label is keyed by pair, so this cannot be
...
PY
)" || { echo "!! skipping pair $pair (no sweep_config.json entry)"; echo; rc=1; continue; }
    eval "$CFG"

    IDX="target_sets/${MODEL}_${ATTACK}_${pair}.json"
    [ -s "$IDX" ] || {
        echo "!! target file $IDX missing -- run ours.sh once to regenerate it"
        echo "!! skipping pair $pair"; echo; rc=1; continue; }
...
# non-zero if any pair was skipped, so a bad config is visible in $?
exit $rc
