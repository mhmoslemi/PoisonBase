#!/usr/bin/env bash
#
# Resume 1 of 4 -- ResNet20BN / gradmatch / dog-bird / budget 0.04 / tgt14
#
# The original run (sel_dpp_grad3_.sh, job 4794909 on kn137) was NOT killed by
# the terminal disconnect at 07:47 -- it kept going and died at 11:47 when the
# allocation hit walltime, mid-craft on target 9059.
#
# State on disk (ours_result/CIFAR10_ResNet20BN_gradmatch_ours_dog-bird_b0.04_
# eps8_seed42_lam1_cosine_seldpp2_ce5_tgt14/):
#     results_rank0.csv   42 rows = 7 targets x 6 victims, all ASR=100.0%
#     poison_cache/       14 files
#     targets 5118 8740 5324 4630 833 2065 7056  done
#     targets 9059 6707 9874                     REMAINING
#
...
#
# Cost: crafting is ~4540 s/target and a victim trial ~131 s, so each remaining
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "r1.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

IDX="target_sets/ResNet20BN_gradmatch_dog-bird.json"
[ -s "$IDX" ] || { echo "r1.sh: target file $IDX missing"; exit 1; }

echo "=== resume | dpp alpha=2.0 | ResNet20BN / gradmatch / dog-bird | budget 0.04 ==="
echo "    7/10 targets already on disk; resuming at target 9059 (8/10)"
echo
