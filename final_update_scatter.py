#!/usr/bin/env python
"""
final_update_scatter.py -- final_update.py plus --scatter_mode.

--scatter_mode (sel_dpp_scatter.sh) stops right after the per-candidate
quantities of the proposed --base ours / --sel_dpp score are computed and,
instead of ranking / crafting / training victims, plots them: for a random
subset of --scatter_points candidates of the poison class,
    x = standardized d + lambda * standardized M   (the score without A)
    y = standardized A                              (backbone-gradient term)
both averaged over the surrogates exactly as _ours_pointwise_score does, so
the real ranking is x - beta * y. Everything below the next line is the
untouched final_update.py, which still runs when --scatter_mode is absent.
...
# --------------------------------------------------------------------------- #
# --scatter_mode: compute the score components, plot them, stop.
#
# _ours_pointwise_score (used by --base ours and, as the DPP quality term, by
# --sel_dpp) builds, PER SURROGATE,
#     rm_s = standardize(d_s) + lam * standardize(M_s)
#     a_s  = standardize(A_s)                      (only with --use_jacobian_score)
# and ranks by the surrogate mean of  rm_s - beta * a_s.  scatter_components
# below is that same loop with the two parts kept separate (and A always
# computed, since the whole point is to look at it), so x = mean_s rm_s and
# y = mean_s a_s are exactly the two quantities the selector trades off.
# Nothing is selected, crafted, or trained. The original select_base_* code
# above is untouched.
# --------------------------------------------------------------------------- #

@torch.no_grad()
def scatter_components(nets, images_norm, labels, x_t_norm, y_adv, lam, device,
                       base_dist='l2', bs=512, jacobian_batch_size=64):
    """Per-candidate (rm, a) of the proposed score for one target, kept apart.

    Returns cls_idx (train indices of the poison class) and five tensors over
    that pool: rm and a as defined in the block comment above, plus the raw
    (un-standardized) surrogate means of d, M and A for the CSV.
    """
    cls_idx = (labels == y_adv).nonzero(as_tuple=True)[0]
    cand = images_norm[cls_idx]
    n = len(cls_idx)
    rm = torch.zeros(n, device=device)
    a = torch.zeros(n, device=device)
    d_raw = torch.zeros(n, device=device)
    m_raw = torch.zeros(n, device=device)
    a_raw = torch.zeros(n, device=device)
    for surrogate_idx, net in enumerate(nets):
        states = [(module, module.training) for module in net.modules()]
        net.eval()
        try:
            ...
            _restore_training_states(states)
    k = float(len(nets))
    return cls_idx, rm / k, a / k, d_raw / k, m_raw / k, a_raw / k
...
_SCATTER_XLABEL = (r'$\mathrm{std}(d_i) + \lambda\,\mathrm{std}(M_i)$'
...
def _scatter_axes(ax, x, y, title, beta):
    ax.scatter(x, y, s=9, alpha=0.45, color=_SCATTER_COLOR, edgecolors='none',
               rasterized=True)
    ax.axhline(0.0, color='#b0b0b0', linewidth=0.7, zorder=0)
    ax.axvline(0.0, color='#b0b0b0', linewidth=0.7, zorder=0)
    ax.grid(color='#e2e2e2', linewidth=0.6)
    ax.set_title(title)
    ax.text(0.03, 0.97,
            'n = %d\nPearson = %.3f\nSpearman = %.3f\nscore = x - %g y'
            % (len(x), _pearson(x, y), _spearman(x, y), beta),
            transform=ax.transAxes, ha='left', va='top', fontsize=8,
            bbox={'facecolor': 'white', 'alpha': 0.9, 'edgecolor': '#cccccc',
                  'boxstyle': 'round,pad=.35'})


def _save_fig(fig, base_path, formats, dpi):
    written = []
    for ext in formats:
        p = '%s.%s' % (base_path, ext)
...
def scatter_main(args):
    """--scatter_mode entry point.

    Same dataset, same cached surrogates (and clean victims, when
    --clean_baseline is given, since they rank the targets exactly as a real
    run does), same targets as the matching sel_dpp.sh run -- the run dir is
    'SCATTER_' + the real run name, so the pairing is by name. Per target the
    (rm, a) components are computed over the WHOLE poison-class pool, then ONE
    seeded random subset of --scatter_points candidates (the same subset for
    every target, so the panels are comparable) is plotted x = rm, y = a.

    Writes into <out_dir>/SCATTER_<run name>/:
    ...
    run_dir = os.path.join(args.out_dir, 'SCATTER_' + build_run_name(args))
    os.makedirs(run_dir, exist_ok=True)
    _LOG_PATH = os.path.join(run_dir, 'log.txt')
    log('=== [scatter_mode] run start: %s on %s (no selection, no crafting, '
        'no victim training) ===' % (build_run_name(args),
                                    ('cuda %s' % gpus) if gpus else 'cpu'))
    log('args: %s' % json.dumps(vars(args), sort_keys=True))
    beta = getattr(args, 'jacobian_weight', 1.0)
    if getattr(args, 'use_jacobian_score', False):
        log('Jacobian score: enabled in this config, weight=%g -> the real ranking '
            'is x - %g * y' % (beta, beta))
    else:
        log('Jacobian score: disabled in this config (real ranking is x alone); '
            'A is computed here anyway because it is the y axis')
    ...
    log('N_total=%d budget=%g -> N_p=%d poisons (not selected here), '
        'y_adv=%d(%s) target_class=%d(%s)'
    ...
    # targets: identical to main() -- clean victims rank them when
    # --clean_baseline is on (the sel_dpp.sh default), a pinned file wins anyway
    clean_victims = []
    if args.clean_baseline:
        log('=== clean victims (%d x %s), only to rank targets ===' % (args.num_victims, args.model))
    ...
    del clean_victims, rank_nets
    gc.collect()
    if gpus:
        torch.cuda.empty_cache()

    # one seeded random subset of the poison-class pool, shared by all targets
    pool = (train_labs == y_adv).nonzero(as_tuple=True)[0]
    n_pool = len(pool)
    n_points = min(args.scatter_points, n_pool)
    sub_gen = torch.Generator(device='cpu').manual_seed(args.seed * 7919 + 1)
    subset = torch.randperm(n_pool, generator=sub_gen)[:n_points].sort().values
    subset_np = subset.numpy()
    log('  poison-class pool %d candidates -> plotting a seeded random subset of %d'
        % (n_pool, n_points))

    plt = _scatter_style()
    formats = [f.strip() for f in args.scatter_formats.split(',') if f.strip()]
    per_target = {}
    npz = {'subset_pool_pos': subset_np, 'pool_train_idx': pool.cpu().numpy()}
    all_x, all_y = [], []
    lam = args.lambda_margin

    for k, tidx in enumerate(targets):
        x_t_norm = test_imgs[tidx]
        set_seed(args.seed * 100003 + int(tidx))     # same reseed prepare_poisons uses
        t0 = time.time()
        cls_idx, rm, a, d_raw, m_raw, a_raw = scatter_components(
            sel_nets, train_imgs, train_labs, x_t_norm, y_adv, lam, device,
            base_dist=args.base_dist,
            jacobian_batch_size=getattr(args, 'jacobian_batch_size', 64))
        assert torch.equal(cls_idx, pool)
        rm_np, a_np = rm.cpu().numpy(), a.cpu().numpy()
        d_np, m_np, araw_np = d_raw.cpu().numpy(), m_raw.cpu().numpy(), a_raw.cpu().numpy()
        x, y = rm_np[subset_np], a_np[subset_np]
        pear, spear = _pearson(x, y), _spearman(x, y)
        pear_full, spear_full = _pearson(rm_np, a_np), _spearman(rm_np, a_np)
        log('  target %d (%d/%d): %d candidates in %.0f s | subset Pearson=%.3f '
            'Spearman=%.3f | full-pool Pearson=%.3f Spearman=%.3f'
            % (tidx, k + 1, len(targets), len(cls_idx), time.time() - t0,
               pear, spear, pear_full, spear_full))

        npz['rm_target%d' % tidx] = rm_np
        npz['a_target%d' % tidx] = a_np
        npz['d_raw_target%d' % tidx] = d_np
        npz['m_raw_target%d' % tidx] = m_np
        npz['a_raw_target%d' % tidx] = araw_np
        per_target[int(tidx)] = {
            'target_score': target_scores.get(tidx, ''),
            'subset_pearson': pear, 'subset_spearman': spear,
            'full_pool_pearson': pear_full, 'full_pool_spearman': spear_full,
        }
        all_x.append(x)
        all_y.append(y)

        with open(os.path.join(run_dir, 'points_target%d.csv' % tidx), 'w',
                  newline='') as f:
            w = csv.writer(f)
            w.writerow(['train_idx', 'x_rm', 'y_a', 'd_raw', 'm_raw', 'a_raw'])
            for j in subset_np:
                w.writerow([int(pool[j]), float(rm_np[j]), float(a_np[j]),
                            float(d_np[j]), float(m_np[j]), float(araw_np[j])])

        fig, ax = plt.subplots(figsize=(6.0, 5.0), constrained_layout=True)
        _scatter_axes(ax, x, y, '%s / %s / %s  target %d' % (args.model, args.attack,
                                                            args.class_pair, tidx), beta)
        ax.set_xlabel(_SCATTER_XLABEL)
        ax.set_ylabel(_SCATTER_YLABEL)
        for p in _save_fig(fig, os.path.join(run_dir, 'scatter_target%d' % tidx),
                           formats, args.scatter_dpi):
            log('  wrote %s' % p)
        plt.close(fig)

    # grid: one panel per target, shared axes
    n_t = len(targets)
    ncols = min(4, n_t)
    nrows = int(math.ceil(n_t / float(ncols)))
    fig, axes = plt.subplots(nrows, ncols, figsize=(4.2 * ncols, 3.8 * nrows),
                             sharex=True, sharey=True, constrained_layout=True,
                             squeeze=False)
    for k, tidx in enumerate(targets):
        ax = axes[k // ncols][k % ncols]
        _scatter_axes(ax, all_x[k], all_y[k], 'target %d' % tidx, beta)
    for k in range(n_t, nrows * ncols):
        axes[k // ncols][k % ncols].set_axis_off()
    for r in range(nrows):
        axes[r][0].set_ylabel(_SCATTER_YLABEL)
    for c in range(ncols):
        axes[nrows - 1][c].set_xlabel(_SCATTER_XLABEL)
    fig.suptitle('%s / %s / %s  (%d random candidates per target)'
                 % (args.model, args.attack, args.class_pair, n_points))
    for p in _save_fig(fig, os.path.join(run_dir, 'scatter_grid'), formats,
                       args.scatter_dpi):
        log('  wrote %s' % p)
    plt.close(fig)

    # pooled: the subsets of all targets in one panel
    px, py = np.concatenate(all_x), np.concatenate(all_y)
    fig, ax = plt.subplots(figsize=(6.0, 5.0), constrained_layout=True)
    _scatter_axes(ax, px, py, '%s / %s / %s  all %d targets pooled'
                  % (args.model, args.attack, args.class_pair, n_t), beta)
    ax.set_xlabel(_SCATTER_XLABEL)
    ax.set_ylabel(_SCATTER_YLABEL)
    for p in _save_fig(fig, os.path.join(run_dir, 'scatter_pooled'), formats,
                       args.scatter_dpi):
        log('  wrote %s' % p)
    plt.close(fig)
    ...
    summary = {
        ...
        'model': args.model, 'attack': args.attack, 'class_pair': args.class_pair,
        'base': args.base, 'sel_dpp': bool(args.sel_dpp), 'sel_alpha': args.sel_alpha,
        'lambda_margin': lam, 'base_dist': args.base_dist,
        'use_jacobian_score': bool(getattr(args, 'use_jacobian_score', False)),
        'jacobian_weight': beta,
        'jacobian_batch_size': getattr(args, 'jacobian_batch_size', 64),
        'jacobian_backend': _jacobian_backend_metadata(),
        'budget': args.budget, 'num_poisons': N_p,
        'num_surrogates': len(sel_nets), 'pool_size': n_pool,
        'scatter_points': n_points, 'seed': args.seed,
...
def parse_args(argv=None):
    ...
    # --- scatter mode (final_update_scatter.py only) ---------------------------
    p.add_argument('--scatter_mode', action='store_true', default=False,
    ...
    if args.scatter_mode:
        ...
        if args.sel_mode not in (None, 'dpp'):
            p.error('--scatter_mode only covers plain ours and --sel_dpp, whose '
                    'pointwise score is d + lam*M - beta*A; got --sel_%s' % args.sel_mode)
        if exact_alignment or a_minus_mr or sel_component or args.sel_criterion \
                or args.base_topr:
            p.error('--scatter_mode only covers plain ours and --sel_dpp (the '
                    'selectors whose score is d + lam*M - beta*A)')
    return args


if __name__ == '__main__':
    _args = parse_args()
    if _args.scatter_mode:
        scatter_main(_args)
    else:
        main(_args)
