...
names=set(C)|set(L)
def merge(a,b):  # b fills / overrides? -> b wins on conflict
...
for n in names:
    c=C.get(n,{}); l=L.get(n,{})
    variants[n]={'csv':c,'log':l,'csv_first':merge(l,c),'log_first':merge(c,l)}
...
for k,v in TEX.items():
    M,A,P,B=k.split('|'); B=float(B)
    tv=(float(v[0]),float(v[1]))
    best=None
    for n,tgt in cell.get((M,A,P,B,'RAND'),[]):
        for vk,d in variants[n].items():
            s=ms(d)
            if not s: continue
            if abs(round(s[0],1)-tv[0])<0.05 and abs(round(s[1],1)-tv[1])<0.05:
                sc=(s[2], vk=='log_first', vk=='csv_first')
                if best is None or sc>best[0]: best=(sc,n,vk,tgt,s)
    if best: sel[k]=(best[1],best[2],best[3],best[4])
    else: miss.append((k,tv,[(n,tgt,{vk:(round(ms(d)[0],1),round(ms(d)[1],1),ms(d)[2]) for vk,d in variants[n].items() if ms(d)}) for n,tgt in cell.get((M,A,P,B,'RAND'),[])]))
json.dump({k:(v[0],v[1],v[2]) for k,v in sel.items()},open(ROOT+'/.ci_work/rand_sel.json','w'))
print("matched",len(sel),"/",len(TEX))
for k,tv,c in miss:
    print("MISS",k,tv)
