#!/usr/bin/env bash
#
# tab:augmentation-robustness -- RandAugment + Crop+Flip -- dog-bird / GM / $2e-2$
#
# Cells filled by this script (ResNet20BN, Random and DPP side by side):
#
#     dog-bird & GM & $2e-2$ & Random & <Crop+Flip> <RandAugment>
#     dog-bird & GM & $2e-2$ & DPP    & <Crop+Flip> <RandAugment>
#
...
#
#   sh aug02.sh              # as configured
#   DRY_RUN=1 sh aug02.sh    # print the defense.py commands and stop

MODEL="${MODEL:-ResNet20BN}" \
CLASS_PAIR="dog-bird" ATTACK="gradmatch" \
BUDGETS="0.02" SELS="random dpp" PAIR_SELS="random dpp" \
