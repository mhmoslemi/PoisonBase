#!/usr/bin/env bash
#
# cv4 -- ConvNetBN augmentation shard: FC / frog-airplane
#
# Fills these rows of aug_table_convnet.tex (Random and DPP side by side, both
# budgets, all three augmentations):
#
#     frog--airplane & FC & $2{\times}10^{-3}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#     frog--airplane & FC & $1{\times}10^{-2}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#
# RUN cv0-2.sh FIRST (it is the half that covers this pair). The Random arm has no cached poisons for this combo, and
...
#
#   sh cv4.sh
#   DRY_RUN=1 sh cv4.sh    # print the defense.py commands and stop
...
tgt = json.load(open('sweep_config.json'))['difficulty']['ConvNetBN']['fc']['frog-airplane']
def ns(base, b, dpp):
    return argparse.Namespace(dataset='CIFAR10', model='ConvNetBN', attack='fc',
        base=base, class_pair='frog-airplane', budget=b, epsilon=0.0313725, seed=42,
...
for b in (0.002, 0.01):
    ...
    if n < len(have['dpp']) or n < 5:
        bad.append(b)
if bad:
    sys.exit('cv4: budget(s) %s do not have the full paired target set yet -- let '
             'cv0-2.sh finish crafting the Random arm first. aug.sh pins the SORTED '
