#!/usr/bin/env bash
#
# Resume: VGG13BN / sapa (worst, sigma=0.05) / random base / dog-bird
#         budget 0.04 / tgt50
#
...
#   -> the 6 victim trials of target 9731, plus its craft
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "sar3.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_VGG13BN_sapa_random_dog-bird_b0.04_eps8_seed42_worst0.05_ce5_tgt50
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== sar3 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== sar3 | sapa worst sigma=0.05 | VGG13BN / random / dog-bird | budget 0.04 ==="
