...
# "ConvNet ... SAPA random" row(s) of table.tex.
#
# The 4e-2 runs are NOT here: they are already running. This shard covers
# budgets 0.01 for ConvNetBN / dog-bird + frog-airplane.
#
...
#
#   ConvNet dog-bird      b0.01  10 crafts x  528 s + 60 trials x  50 s   ~2.30 h
#   ConvNet frog-airplane b0.01  10 crafts x  528 s + 60 trials x  50 s   ~2.30 h
#   ------------------------------------------------------------------
#   L40S  ~4.6 h      H100  ~2.6 h  (range 2.1-2.7 h, see note)
#
...
#
# Targets come from the pinned target_sets/ConvNetBN_gradmatch_<pair>.json, which
...
#
#     sh sa1.sh
#     BUDGETS="0.001" sh sa1.sh     # narrow it if the allocation is short

set -u

MODEL_=ConvNetBN
PAIRS_="dog-bird frog-airplane"
BUDGETS_="${BUDGETS:-0.01}"

cd /home/mmoslem3/scratch/attack_if

for pair in $PAIRS_; do
    echo "=== sa1 | sapa worst sigma=0.05 | $MODEL_ / random / $pair | budgets $BUDGETS_ ==="
