#!/usr/bin/env bash
#
# tab:augmentation-robustness -- Cutout -- frog-airplane FC (both budgets)
#
# Cells filled by this script (ResNet20BN, Random and DPP side by side):
#
#     frog--airplane & FC & $2{\\times}10^{-3}$ and $1{\\times}10^{-2}$ & Random, DPP & <Cutout>
#
...
#
# Estimated ~3.1 h (4 runs x 20 trials) on an L40S (ResNet20BN, 130 s/victim measured; Crop+Flip and
# Cutout ~1.06x that, RandAugment ~1.4x).
#
#   sh aug11.sh              # as configured
#   DRY_RUN=1 sh aug11.sh    # print the defense.py commands and stop
