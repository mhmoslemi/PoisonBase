#!/usr/bin/env bash
#
# cv0-2 -- PREREQUISITE for cv2.sh and cv4.sh. Recrafts the Random arm of the
# ConvNetBN fc/gradmatch sweep for the frog-airplane pair.
#
...
# target_sets/ConvNetBN_<attack>_frog-airplane.json -- the same 10 the DPP arm
...
# CELLS="1 2" gets both fc cells done in ~1.2 h, which is all cv4/cv4 need.
#
# Nothing already cached is redone -- a rerun after a wall-clock kill resumes.
#
#   sh cv0-2.sh
#   CELLS="1 2" sh cv0-2.sh     # run only some cells (1..4, cheapest first)
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "cv0-2.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

# "<attack> <budget> <difficulty label>" -- cheapest first
cell_1="fc        0.002  20"
cell_2="fc        0.01   20"
cell_3="gradmatch 0.002  35"
cell_4="gradmatch 0.02   35"

for c in $CELLS; do
    eval "spec=\${cell_$c:-}"
    [ -n "$spec" ] || { echo "cv0-2.sh: no such cell '$c' (expected 1..4)"; exit 1; }
    # shellcheck disable=SC2086
    set -- $spec
    attack=$1; bud=$2; tg=$3

    echo "=== cv0-2 cell $c | ConvNetBN / $attack / frog-airplane / b$bud | crafting Random arm ==="
    python final_update.py \
        ...
        --target_idx_file "target_sets/ConvNetBN_${attack}_frog-airplane.json" \
        --num_victims 4 --victim_epochs 50 --victim_lr 0.1 --victim_bs 125 \
        --victim_decay 40 --victim_wd 0.0 \
        --clean_baseline || exit 1
done

echo "=== cv0-2.sh finished (cells: $CELLS) ==="
echo "when cv0-1 and cv0-2 are both done, run cv2.sh and cv4.sh"
