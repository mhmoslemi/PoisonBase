#!/usr/bin/env bash
#
# ca3 -- ConvNetBN / FC / dog-bird / $1{\times}10^{-2}$ / Random
#
# Fills these 3 cells of aug_table.tex (the ConvNetBN block):
#
#     dog--bird & FC & $1{\times}10^{-2}$ & Random & (No Aug.) & <Crop+Flip> <RandAugment> <Cutout>
#
...
#
#   sh ca3.sh
#   DRY_RUN=1 sh ca3.sh    # print the defense.py commands and stop
...
def ns(base, dpp):
    return argparse.Namespace(dataset='CIFAR10', model='ConvNetBN', attack='fc', base=base,
        class_pair='dog-bird', budget=0.01, epsilon=0.0313725, seed=42, lambda_margin=1.0,
...
print('  b0.01  random=%d dpp=%d -> %d paired' % (len(have['random']), len(have['dpp']), n))
if n < len(have['dpp']) or n < 5:
    sys.exit('ca3: the Random arm is not fully crafted for b0.01 (%d of %d paired). '
             'Run cv0-1.sh first.' % (n, len(have['dpp'])))
PYCHK

MODEL=ConvNetBN ATTACK=fc CLASS_PAIR="dog-bird" \
    BUDGETS="0.01" SELS="random" PAIR_SELS="random dpp" \
    AUGS="standard randaug cutout" NUM_TARGETS=5 NUM_VICTIMS=4 \
    sh ./aug.sh || exit 1

echo "=== ca3.sh finished ==="
