#!/usr/bin/env bash
#
# appendix.tex tab:utility-defense -- FRIENDS on dog-bird.
#
...
#
#   sh appendix/final/b3-defense-friends-dog-bird.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "b3-defense-friends-dog-bird.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

PAIRS=dog-bird sh appendix/ap5-c.sh
echo "=== b3-defense-friends-dog-bird.sh finished ==="
