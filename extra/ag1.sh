#!/usr/bin/env bash
#
# ag1 -- the four ConvNet Cutout cells, plus the one ResNet20BN cell left over
#
# Cells this fills in aug_table.tex:
#
#     ConvNetBN  b0.002  Random  Cutout      0/20 done, 20 left
#     ConvNetBN  b0.002  DPP     Cutout      0/20 done, 20 left
#     ConvNetBN  b0.01   Random  Cutout      0/20 done, 20 left
#     ConvNetBN  b0.01   DPP     Cutout      0/20 done, 20 left
#     ResNet20BN b0.02   DPP     Crop+Flip  16/20 done,  4 left
#
...
#
# COST:
#   80 ConvNet cutout trials x ~41 s  = 0.91 h
#    4 ResNet20 Crop+Flip trials x 137 s = 0.15 h
#   -> ~1.1 h.
#
#   The ConvNet cutout rate is the one number here that is not measured: no
#   ConvNet cutout run exists yet. It is ConvNet standard (38 s, measured on the
#   H100 in these very runs) scaled by the ResNet cutout/standard ratio
#   (146/137 = 1.07). Treat it as +-20%.
#
#   sh ag1.sh
#   DRY_RUN=1 sh ag1.sh    # print the defense.py commands and stop
...
MODEL=ConvNetBN ATTACK=fc CLASS_PAIR="dog-bird" \
    BUDGETS="0.002 0.01" SELS="random dpp" PAIR_SELS="random dpp" \
    AUGS="cutout" NUM_TARGETS=5 NUM_VICTIMS=4 \
    sh ./aug.sh || exit 1

# The ResNet20BN block still shows -- for GM / 2e-2 / DPP / Crop+Flip: 16/20 done,
# only target 5324 victims 0-3 are missing. ca1.sh had this tacked on the end and
# never reached it (its first call was killed on the wall clock first).
