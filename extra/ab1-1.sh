#!/usr/bin/env bash
#
# ablation.tex tab:alpha-ablation (alpha), shard 1 of 8
#
#     ResNet20BN  b0.01   Greedy (lam=1, no dpp)
#     ConvNetBN   b0.01   dpp alpha=0.25
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab1-1.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp0.25_worst0.05_ce5_tgt70 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 0.25

echo "=== ab1-1.sh finished ==="
