...
#
#   sh appendix/final/c5-ladder-resnet-fc-9to11.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "c5-ladder-resnet-fc-9to11.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

MODELS=ResNet20BN ATTACKS=fc CRITS="relevance greedy dpp" sh appendix/fin3-ladder.sh
echo "=== c5-ladder-resnet-fc-9to11.sh finished ==="
