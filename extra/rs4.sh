...
#   -> 3 craft(s) x 4627 s + 18 trial(s) x 100 s ~= 4.4 h
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "rs4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_VGG13BN_sapa_ours_frog-airplane_b0.04_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt12
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== rs4 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== rs4 | sapa dpp | VGG13BN / frog-airplane / budget 0.04 ==="
