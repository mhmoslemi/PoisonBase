#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 3 of 16
#
#     ResNet20BN  b0.01   Greedy lambda=0.5
#     ResNet20BN  b0.005  DPP lambda=4 (alpha=2)
#     ConvNetBN   b0.005  Greedy lambda=1
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-3.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam0.5_cosine_worst0.05_ce5_tgt14 \
    --lambda_margin 0.5

M=ResNet20BN; B=0.005; TGT=14; NSUR=20
LABEL="DPP lambda=4 (alpha=2)"
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.005_eps8_seed42_lam4_cosine_seldpp2_worst0.05_ce5_tgt14 \
