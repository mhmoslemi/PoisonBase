#!/usr/bin/env bash
#
# ablation.tex tab:ensemble-size-ablation (K), shard 8 of 8
#
#     ResNet20BN  b0.005  DPP K=5
#     ConvNetBN   b0.01   DPP K=1
#     ConvNetBN   b0.005  DPP K=3
#
# 5 targets x 4 victims = 20 trials per run, targets pinned to the first 5 of
# target_sets/<model>_gradmatch_dog-bird.json in file order. ~3.0 h.
#
#   sh ab3-8.sh
#   DRY_RUN=1 sh ab3-8.sh
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab3-8.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
LABEL="DPP K=5"
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.005_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt14_K5 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 2.0 --sel_K 5

M=ConvNetBN; B=0.01; TGT=70; NSUR=20
LABEL="DPP K=1"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt70_K1 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 2.0 --sel_K 1

M=ConvNetBN; B=0.005; TGT=70; NSUR=20
LABEL="DPP K=3"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.005_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt70_K3 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 2.0 --sel_K 3

echo "=== ab3-8.sh finished ==="
