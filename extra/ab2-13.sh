#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 13 of 16
#
#     ResNet20BN  b0.01   DPP lambda=100 (alpha=2)
#     ConvNetBN   b0.01   Greedy lambda=2
#     ConvNetBN   b0.005  DPP lambda=0 (alpha=2)
#     ConvNetBN   b0.005  DPP lambda=10 (alpha=2)
#
# 5 targets x 4 victims = 20 trials per run, targets pinned to the first 5 of
# target_sets/<model>_gradmatch_dog-bird.json in file order. ~4.3 h.
#
#   sh ab2-13.sh
#   DRY_RUN=1 sh ab2-13.sh
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-13.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
LABEL="DPP lambda=100 (alpha=2)"
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam100_cosine_seldpp2_worst0.05_ce5_tgt14 \
    --lambda_margin 100 --sel_dpp --sel_alpha 2.0

M=ConvNetBN; B=0.01; TGT=70; NSUR=20
LABEL="Greedy lambda=2"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam2_cosine_worst0.05_ce5_tgt70 \
    --lambda_margin 2

M=ConvNetBN; B=0.005; TGT=70; NSUR=20
LABEL="DPP lambda=0 (alpha=2)"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.005_eps8_seed42_lam0_cosine_seldpp2_worst0.05_ce5_tgt70 \
    --lambda_margin 0 --sel_dpp --sel_alpha 2.0

M=ConvNetBN; B=0.005; TGT=70; NSUR=20
LABEL="DPP lambda=10 (alpha=2)"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.005_eps8_seed42_lam10_cosine_seldpp2_worst0.05_ce5_tgt70 \
    --lambda_margin 10 --sel_dpp --sel_alpha 2.0

echo "=== ab2-13.sh finished ==="
