#!/usr/bin/env bash
#
# appendix.tex tab:augmentation-aware -- SAPA under RandAugment, dpp selection.
#
...
#
#   sh appendix/final/a5-aug-randaug-sapa-dpp.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "a5-aug-randaug-sapa-dpp.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

STEP=ra_sapa SELS=dpp sh appendix/ap4-augaware.sh
echo "=== a5-aug-randaug-sapa-dpp.sh finished ==="
