...
#   38/60 trials done, 10 of 10 targets crafted
#   -> 3 craft(s) x 4549 s + 22 trial(s) x 130 s ~= 4.6 h
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "rs1.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.04_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt14
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== rs1 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== rs1 | sapa dpp | ResNet20BN / dog-bird / budget 0.04 ==="
