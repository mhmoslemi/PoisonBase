#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 10 of 16
#
#     ResNet20BN  b0.01   Greedy lambda=10
#     ConvNetBN   b0.01   Greedy lambda=0.5
#     ConvNetBN   b0.01   Greedy lambda=100
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-10.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam10_cosine_worst0.05_ce5_tgt14 \
    --lambda_margin 10

M=ConvNetBN; B=0.01; TGT=70; NSUR=20
LABEL="Greedy lambda=0.5"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam0.5_cosine_worst0.05_ce5_tgt70 \
    --lambda_margin 0.5

M=ConvNetBN; B=0.01; TGT=70; NSUR=20
LABEL="Greedy lambda=100"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam100_cosine_worst0.05_ce5_tgt70 \
    --lambda_margin 100

echo "=== ab2-10.sh finished ==="
