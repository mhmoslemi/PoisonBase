#!/usr/bin/env bash
#
# appendix.tex, tab:cross-dataset -- chunk 3 of 4.
#
# surrogates 16-19, then the first TinyImageNet attack run
#
# ~6.8 h, from measured rates: a ResNet18BN TinyImageNet clean victim is 2327 s,
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ap2-3.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
echo
echo "########## TinyImageNet surrogates 16,17,18,19 ##########"
...
run "TinyImageNet_${TINY_MODEL}_gradmatch_random_n01443537-n01629819_b0.001_eps8_seed42_ce5" \
