#!/usr/bin/env bash
#
# cv2 -- ConvNetBN augmentation shard: GM / frog-airplane
#
# Fills these rows of aug_table_convnet.tex (Random and DPP side by side, both
# budgets, all three augmentations):
#
#     frog--airplane & GM & $2{\times}10^{-3}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#     frog--airplane & GM & $2{\times}10^{-2}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#
...
#
#   sh cv2.sh
#   DRY_RUN=1 sh cv2.sh    # print the defense.py commands and stop
...
tgt = json.load(open('sweep_config.json'))['difficulty']['ConvNetBN']['gradmatch']['frog-airplane']
def ns(base, b, dpp):
...
for b in (0.002, 0.02):
    ...
    if n < len(have['dpp']) or n < 5:
        bad.append(b)
if bad:
    sys.exit('cv2: budget(s) %s do not have the full paired target set yet -- let '
