...
#
#   sh appendix/final/c4-ladder-resnet-fc-5to8.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "c4-ladder-resnet-fc-5to8.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

MODELS=ResNet20BN ATTACKS=fc CRITS="el2n boundary pixel featsim" sh appendix/fin3-ladder.sh
echo "=== c4-ladder-resnet-fc-5to8.sh finished ==="
