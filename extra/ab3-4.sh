#!/usr/bin/env bash
#
# ablation.tex tab:ensemble-size-ablation (K), shard 4 of 8
#
#     ResNet20BN  b0.01   DPP K=10
#     ConvNetBN   b0.01   DPP K=50
#
# 5 targets x 4 victims = 20 trials per run, targets pinned to the first 5 of
# target_sets/<model>_gradmatch_dog-bird.json in file order. ~3.1 h.
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab3-4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
LABEL="DPP K=10"
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt14_K10 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 2.0 --sel_K 10

M=ConvNetBN; B=0.01; TGT=70; NSUR=50
LABEL="DPP K=50"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt70_K50 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 2.0 --sel_K 50

echo "=== ab3-4.sh finished ==="
