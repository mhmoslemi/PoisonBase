#!/usr/bin/env bash
#
# cv3 -- ConvNetBN augmentation shard: FC / dog-bird
#
# Fills these rows of aug_table_convnet.tex (Random and DPP side by side, both
# budgets, all three augmentations):
#
#     dog--bird & FC & $2{\times}10^{-3}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#     dog--bird & FC & $1{\times}10^{-2}$ & Random/DPP & .. & <Crop+Flip> <RandAugment> <Cutout>
#
...
#
#   sh cv3.sh
#   DRY_RUN=1 sh cv3.sh    # print the defense.py commands and stop
...
for b in (0.002, 0.01):
    ...
    if n < len(have['dpp']) or n < 5:
        bad.append(b)
if bad:
    sys.exit('cv3: budget(s) %s do not have the full paired target set yet -- let '
