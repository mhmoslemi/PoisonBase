...
# (what makes it sapa rather than gradmatch), and --craft_lowmem --fast_gradmatch.
#
...
#   42/60 trials done, 10 of 10 targets crafted
#   -> 3 craft(s) x 4625 s + 18 trial(s) x 100 s ~= 4.4 h
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "rs3.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_VGG13BN_sapa_ours_dog-bird_b0.04_eps8_seed42_lam1_cosine_seldpp2_worst0.05_ce5_tgt50
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== rs3 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== rs3 | sapa dpp | VGG13BN / dog-bird / budget 0.04 ==="
python final_update.py \
    --dataset "$DATASET" --data_path "$DATA_PATH" --seed "$SEED" \
    --cache_dir "$CACHE_DIR" --out_dir "$OUT_DIR" \
    --model VGG13BN --attack sapa --base ours \
