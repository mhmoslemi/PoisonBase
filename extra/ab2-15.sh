#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 15 of 16
#
#     ResNet20BN  b0.005  DPP lambda=0 (alpha=2)
#     ResNet20BN  b0.005  Greedy lambda=1
#     ConvNetBN   b0.01   Greedy lambda=4
#     ConvNetBN   b0.005  DPP lambda=100 (alpha=2)
#
# 5 targets x 4 victims = 20 trials per run, targets pinned to the first 5 of
# target_sets/<model>_gradmatch_dog-bird.json in file order. ~4.4 h.
#
#   sh ab2-15.sh
#   DRY_RUN=1 sh ab2-15.sh
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-15.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
