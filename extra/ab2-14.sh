#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 14 of 16
#
#     ResNet20BN  b0.005  Greedy lambda=0
#     ResNet20BN  b0.005  DPP lambda=0.5 (alpha=2)
#     ConvNetBN   b0.01   DPP lambda=2 (alpha=2)
#     ConvNetBN   b0.005  Greedy lambda=100
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-14.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
