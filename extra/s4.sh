...
#   40/60 trials done -> 20 victim trials over targets 7056 9059 6707 9874
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "s4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_ResNet20BN_sapa_random_dog-bird_b0.005_eps8_seed42_worst0.05_ce5_tgt14
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== s4 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== s4 | sapa worst sigma=0.05 | ResNet20BN / random / dog-bird | budget 0.005 ==="
