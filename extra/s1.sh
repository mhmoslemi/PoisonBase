#!/usr/bin/env bash
#
# Resume 1 of 2 for the pair of sapa runs killed in crash_again.txt.
#
#   ConvNetBN / sapa / random / dog-bird / budget 0.005 / tgt70
#
...
#   -> 12 victim trials (targets 1547, 4427) + 2 crafts
#   -> 12 x 50 s + 2 x 243 s ~= 0.30 h
#
# s2.sh is the frog-airplane half; the two are independent, so run them on two
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "s1.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_ConvNetBN_sapa_random_dog-bird_b0.005_eps8_seed42_worst0.05_ce5_tgt70
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== s1 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== s1 | sapa worst sigma=0.05 | ConvNetBN / random / dog-bird | budget 0.005 ==="
python final_update.py \
    ...
    --num_targets 10 --target_select 70 \
    --target_idx_file "target_sets/ConvNetBN_gradmatch_dog-bird.json" \
