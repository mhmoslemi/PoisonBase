#!/usr/bin/env bash
#
# ablation.tex tab:alpha-ablation (alpha), shard 4 of 8
#
#     ResNet20BN  b0.01   dpp alpha=1
#     ConvNetBN   b0.01   dpp alpha=4
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab1-4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
LABEL="dpp alpha=1"
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp1_worst0.05_ce5_tgt14 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 1

M=ConvNetBN; B=0.01; TGT=70; NSUR=20
LABEL="dpp alpha=4"
run CIFAR10_ConvNetBN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam1_cosine_seldpp4_worst0.05_ce5_tgt70 \
    --lambda_margin 1.0 --sel_dpp --sel_alpha 4

echo "=== ab1-4.sh finished ==="
