#!/usr/bin/env bash
#
# ablation.tex tab:lambda-ablation (lambda), shard 6 of 16
#
#     ResNet20BN  b0.01   Greedy lambda=2
#     ResNet20BN  b0.005  Greedy lambda=100
#     ConvNetBN   b0.005  Greedy lambda=4
#
...
if [ -z "$DRY_RUN" ]; then
    python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
        echo "ab2-6.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi
...
run CIFAR10_ResNet20BN_sapa_ours_dog-bird_b0.01_eps8_seed42_lam2_cosine_worst0.05_ce5_tgt14 \
    --lambda_margin 2
