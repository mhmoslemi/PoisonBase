#!/usr/bin/env bash
#
# appendix.tex tab:augmentation-aware -- GM under RandAugment, dpp selection.
#
...
#
#   sh appendix/final/a3-aug-randaug-gm-dpp.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "a3-aug-randaug-gm-dpp.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

STEP=ra_gm SELS=dpp sh appendix/ap4-augaware.sh
echo "=== a3-aug-randaug-gm-dpp.sh finished ==="
