...
#
#   sh appendix/final/c6-ladder-resnet-gm-1to4.sh
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "c6-ladder-resnet-gm-1to4.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
fi

MODELS=ResNet20BN ATTACKS=gradmatch CRITS="bottom first random grand" sh appendix/fin3-ladder.sh
echo "=== c6-ladder-resnet-gm-1to4.sh finished ==="
