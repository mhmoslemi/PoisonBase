...
#   -> the 6 victim trials of target 2041, plus its craft
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "sar2.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }

TAG=CIFAR10_ResNet20BN_sapa_random_frog-airplane_b0.04_eps8_seed42_worst0.05_ce5_tgt10
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== sar2 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== sar2 | sapa worst sigma=0.05 | ResNet20BN / random / frog-airplane | budget 0.04 ==="
