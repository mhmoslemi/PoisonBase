#!/usr/bin/env bash
#
# Diversity-aware base selection, mode: filter
#   quality gate + farthest-point: keep the best SEL_POOL x N_p by score,
#   then spread inside that pool.  SEL_POOL=1.0 reproduces plain --base ours.
#
...
#
# The run name gets a _sel_pool<value> suffix, so these land in their own
...
#     for p in dog-bird frog-airplane; do MODEL=VGG13BN CLASS_PAIR=$p sh sel_filter.sh; done )

MODEL="${MODEL:-ConvNetBN}"
ATTACK="${ATTACK:-fc}"
CLASS_PAIR="${CLASS_PAIR:-frog-airplane}"
# BUDGETS="${BUDGETS:-0.001 0.002 0.005 0.01 0.02 0.04}"
BUDGETS="${BUDGETS:-0.005}"

# SEL_POOL="${SEL_POOL:-2.5}"
SEL_POOL="${SEL_POOL:-2}"
...
echo "=== filter selection | $MODEL / $ATTACK / $CLASS_PAIR | SEL_POOL=$SEL_POOL ==="
...
for bug in $BUDGETS; do
    ...
    python final_update.py \
        ...
        --sel_filter --sel_pool "$SEL_POOL" \
        --num_surrogates 15 --surrogate_epochs 60 --surrogate_decay 35 45 \
