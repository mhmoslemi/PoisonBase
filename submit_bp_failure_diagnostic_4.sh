#!/bin/sh
# Submit the four one-config BP failure-diagnostic jobs.
...
JOB_ROOT="$SOURCE_ROOT/sbatch/bp_failure_diagnostic_20260910"
...
find "$JOB_ROOT" -maxdepth 1 -type f -name 'bpdiag_*.sh' -print | sort > "$JOB_LIST"
