#!/usr/bin/env bash
#
# Diversity-aware base selection, mode: mmr
#   greedy on  score_i + SEL_MU * zscore(max_{j in S} sim(i,j)).
#   SEL_MU is in z-score units, so it is comparable to the score itself.
#   SEL_MU=0 reproduces plain --base ours.
#
...
#
# The run name gets a _sel_mu<value> suffix, so these land in their own
...
#     for p in dog-bird frog-airplane; do MODEL=VGG13BN CLASS_PAIR=$p sh sel_mmr.sh; done )
...
echo "=== mmr selection | $MODEL / $ATTACK / $CLASS_PAIR | SEL_MU=$SEL_MU ==="
...
for bug in $BUDGETS; do
    ...
    python final_update.py \
        ...
        --sel_mmr --sel_mu "$SEL_MU" \
        --num_surrogates 5 --surrogate_epochs 60 --surrogate_decay 35 45 \
