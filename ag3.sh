#!/usr/bin/env bash
#
# ag3 -- ConvNet RandAugment, DPP, 2e-3
#
# Cells this fills in aug_table.tex:
#
#     ConvNetBN  b0.002  DPP  RandAugment  0/20 done, 20 left
#
# This one never ran a single trial: it crashed at startup on an AttributeError
# in build_run_name (defense.py builds its own Namespace, which has no
# sel_model attribute -- added with the cross-architecture flag). Fixed; the
...
#
#   sh ag3.sh
#   DRY_RUN=1 sh ag3.sh    # print the defense.py commands and stop
