#!/usr/bin/env bash
#
# Resume: ResNet20BN / gradmatch / dpp (alpha=2.0) / dog-bird / budget 0.001 / tgt14
#
...
#   48/60 trials done -> 12 victim trials over targets 6707 9874
#   plus 1 craft(s), at this run's own logged rates  ->  ~0.49 h
#
...
python -c 'import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)' || {
    echo "r6.sh: no CUDA device visible -- get a GPU allocation first"; exit 1; }
...
if grep -q "==== $TAG : ASR = " "$OUT_DIR/$TAG/log.txt" 2>/dev/null; then
    echo "=== r6 | already complete:"
    grep -o "ASR = .*====" "$OUT_DIR/$TAG/log.txt" | tail -1
    exit 0
fi

echo "=== r6 | dpp alpha=2.0 | ResNet20BN / gradmatch / dog-bird | budget 0.001 ==="
